  ...
  return (
    <span aria-hidden="true" className="grid shrink-0 grid-cols-3 gap-[3px]">
      {Array.from({ length: 9 }, (_, i) => (
        <span key={i} className="h-[3px] w-[3px] rounded-full bg-admin-fg-faint" />
      ))}
    </span>
  );
}
...
export default function SessionCards() {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {CARDS.map((card) => (
        <div key={card.label} className="rounded-xl border border-admin-line bg-admin-panel p-4">
          <div className="flex items-start justify-between gap-2">
            <span className="text-[11px] font-medium text-admin-fg-muted">{card.label}</span>
            <GripDots />
          </div>

          <div className="mt-1 flex items-center gap-2">
            <span className="text-[22px] font-semibold leading-tight text-admin-fg">{card.value}</span>
